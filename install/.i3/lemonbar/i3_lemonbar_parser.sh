...
while read -r line ; do
  case $line in
    SYS*)
      ...
      wland="%{F${wlan_cback}}${sep_left}%{F${wlan_cicon} B${wlan_cback}} %{T2}${icon_dl}%{F${wlan_cfore} T1} ${wland_v}${stab}"
      wlanu="%{F${wlan_cicon}}${sep_l_left} %{T2}${icon_ul}%{F${wlan_cfore} T1} ${wlanu_v}${stab}"
      if [ "${sys_arr[8]}" == "down" ];then
          wland="%{F${color_red2}}${sep_left}%{F${color_icon} B${color_red2}} %{T1}${wland_v} "
          wlanu=""
      fi
      # eth
      ...
      ethu="%{F${eth_cicon}}${sep_l_left} %{T2}${icon_ul}%{F${eth_cfore} T1} ${ethu_v}${stab}"
      if [ "${sys_arr[10]}" == "down" ];then
